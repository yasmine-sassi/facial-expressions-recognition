...
Supports both PyTorch and Keras models
...
# ==================== LOAD MODEL & DATA ====================
@st.cache_resource
def load_model(model_type='pytorch'):
    """Load trained emotion recognition model (PyTorch or Keras)."""
    try:
        if model_type == 'pytorch':
            ...
            if model_path.exists():
                ...
                try:
                    ...
                    model = get_model('baseline', num_classes=7, device=device)
                    model.load_state_dict(torch.load(model_path, map_location=device))
                    model.eval()
                    return model, device, 'pytorch'
            else:
                st.warning("PyTorch model not found")
            ...
            if model_path.exists():
                ...
                st.warning("TensorFlow model not found")
                return None, None, None
...
def detect_emotions_in_frame(frame, face_cascade, model, device, model_type):
...
with col1:
    st.sidebar.markdown("**Model Framework:**")
    model_framework = st.sidebar.radio(
        "Select framework:",
        ["PyTorch (GPU)", "TensorFlow"],
        label_visibility="collapsed"
    )

# Determine model type
model_type = 'pytorch' if 'PyTorch' in model_framework else 'tensorflow'
...
if model_framework == "PyTorch (GPU)":
    device_info = f"Device: {'GPU 🚀' if torch.cuda.is_available() else 'CPU'}"
    st.sidebar.success(f"✅ {device_info}")
else:
    st.sidebar.info("Framework: TensorFlow/Keras")
...
model, device, loaded_model_type = load_model(model_type)
...
if page == "📹 Webcam Detection":
    ...
    if model is None:
        st.error(f"Model not loaded. Please ensure a trained {model_framework} model is saved correctly.")
    else:
        ...
        if start_button:
            ...
            if not cap.isOpened():
                ...
                st.success(f"Webcam started! Press 'Stop' to finish. Using: {model_framework}")
                ...
                while cap.isOpened() and not stop_button:
                    ...
                    frame_processed, emotions_detected = detect_emotions_in_frame(
                        frame, face_cascade, model, device, loaded_model_type
                    )
...
# ==================== PAGE: ABOUT ====================
    ...
    with col1:
        ...
        **Current Framework:** {model_framework}
        ...
        - **Dual Framework Support**: PyTorch (GPU) or TensorFlow (CPU/GPU)
        ...
        **GPU Support:**
        ...
        1. **Select Framework** (top sidebar)
           - PyTorch (GPU) - Faster with NVIDIA GPU
           - TensorFlow - Compatible with all devices
        ...
        **Model not loading?**
        - Train models using notebooks:
          - Keras: 02_baseline_cnn.ipynb, 03_advanced_model.ipynb
          - PyTorch: 04_pytorch_baseline.ipynb, 05_pytorch_advanced.ipynb
        - For Keras: Save to `../saved_models/best_model.h5`
        - For PyTorch: Save to `../saved_models/pytorch_best_model.pt`
    ...
    <p>Built with ❤️ for Facial Emotion Recognition | PyTorch + Streamlit | 2024-2025</p>
    </div>
    """, unsafe_allow_html=True)
...
<p>Facial Emotion Recognition System | {model_framework} | 2024-2025</p>
</div>
""", unsafe_allow_html=True)
